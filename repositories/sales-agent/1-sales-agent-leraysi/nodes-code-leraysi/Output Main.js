...
console.log("[OutputMain v3.0] Original state loaded, row_id:", originalState.row_id);
...
function extractJson(text) {
  ...
  try {
    ...
    console.log("[OutputMain v3.0] Parse intento 1 falló");
  }
  ...
  try {
    ...
    console.log("[OutputMain v3.0] Parse intento 2 falló");
  }
  ...
  if (jsonMatch) {
    ...
    try {
      ...
      console.log("[OutputMain v3.0] Parse intento 3 falló");
    }
  }
...
if (llmData.output) {
  ...
  console.log("[OutputMain v3.0] ✅ LLM JSON parsed");
} else {
  llmOutput = llmData;
}
...
console.log("[OutputMain v3.0] state_patch keys:", Object.keys(statePatch));
...
for (const [key, value] of Object.entries(statePatch)) {
  ...
  if (protectedFields.includes(key)) {
    console.log(`[OutputMain v3.0] ⚠️ Ignorando campo protegido: ${key}`);
    continue;
  }
  ...
  if (["services_seen", "prices_asked", "deep_interest"].includes(key)) {
    ...
    console.log(`[OutputMain v3.0] Contador ${key}: ${currentVal} → ${mergedState[key]}`);
    continue;
  }
  ...
  if (key === "interests" && Array.isArray(value)) {
    ...
    console.log(`[OutputMain v3.0] Interests merged:`, merged);
    continue;
  }
  ...
  if ((key === "email_ask_ts" || key === "fullname_ask_ts") && value === true) {
    ...
    console.log(`[OutputMain v3.0] ${key} convertido a timestamp:`, timestamp);
    continue;
  }
...
console.log("[OutputMain v3.0] Merged state:");
...
console.log("[OutputMain v3.0] Notes generado:", baserowUpdate.notes);
...
const output = {
  ...
  meta: {
    ...
    version: "leraysi-output@3.0",
    patch_applied: Object.keys(statePatch),
  },
};

console.log("[OutputMain v3.0] ✅ Done");
