...
export const getRequestMeta = (req: Request): RequestMeta => {
  ...
  const fallbackMeta: RequestMeta = {
    ...
    timezone: extra.timezone ?? (req.headers["x-client-timezone"] as string) ?? "",
    screenResolution: extra.screenResolution ?? (req.headers["x-screen-resolution"] as string) ?? "",
    label: extra.label ?? (req.headers["x-client-label"] as string) ?? "",
