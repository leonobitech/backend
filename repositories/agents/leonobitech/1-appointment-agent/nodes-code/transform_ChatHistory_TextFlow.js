...
function stripPrefixes(text, role) {
  ...
  if (role === "user") {
    t = t
      .replace(/^<strong>\s*cliente:\s*<\/strong>\s*/i, "")
      .replace(/^cliente:\s*/i, "");
  } else if (role === "assistant") {
    t = t
      .replace(/^<strong>\s*⋆˚🧚‍♀️?\s*(leraysi|leonobit):?\s*<\/strong>\s*/i, "")
      .replace(/^⋆˚🧚‍♀️?\s*(leraysi|leonobit):?\s*/i, "")
      .replace(/^<strong>\s*(leraysi|leonobit)\s*⋆˚🧚‍♀️?:?\s*<\/strong>\s*/i, "")
      .replace(/^(leraysi|leonobit)\s*⋆˚🧚‍♀️?:?\s*/i, "");
  }
  return t.trim();
}
...
function inferRole(mm) {
  ...
  if (
    /^<\s*strong>\s*cliente\s*:\s*<\/\s*strong>/i.test(body) ||
    /^cliente\s*:/i.test(preview)
  )
    return "user";
  if (
    /leraysi|leonobit/i.test(body + preview) ||
    /🧚‍♀️/.test(body + preview) ||
    /⋆˚/.test(body + preview)
  )
    return "assistant";
...
let raw = items
  .map((it) => {
    ...
    const textRaw = mm.preview
      ? String(mm.preview).trim()
      : cleanHtmlKeepStrong(mm.body || "");
    const text = stripPrefixes(textRaw, role);
...
for (const h of raw) {
  const key = h._id
    ? `id:${h._id}`
    : `h:${h.role}:${h.text}:${minuteBucket(h.ts)}`;
