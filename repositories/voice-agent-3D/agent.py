...
from livekit.plugins import anthropic, deepgram, elevenlabs, lemonslice, silero
...
class VoiceAssistant(Agent):
    def __init__(self) -> None:
        super().__init__(
            instructions="""Eres Leo, un simpático león animado 3D y asistente virtual.
            Eres amigable, cálido y con mucha energía positiva.
            ...
            Siempre usa genero masculino al referirte a ti mismo.
...
async def entrypoint(ctx: agents.JobContext):
    ...
    # TODO: Cambiar voice_id por una voz masculina acorde al personaje del león
    elevenlabs_tts = elevenlabs.TTS(
        voice_id=os.getenv("ELEVENLABS_VOICE_ID", "nTkjq09AuYgsNR8E4sDe"),
    ...
    # ── LemonSlice avatar (lip-synced video participant) ──────
    avatar = None
    # TODO: Reemplazar con URL pública de la imagen del león 3D (368x560px)
    avatar_image_url = os.getenv("LEMONSLICE_AVATAR_IMAGE_URL", "")
    avatar_agent_id = os.getenv("LEMONSLICE_AGENT_ID", "")
    avatar_boot_start = time.monotonic()
    logger.info(f"[PIPELINE] avatar_boot_start t={avatar_boot_start - session_start_time:.3f}s")
    try:
        avatar_kwargs = {}
        if avatar_agent_id:
            avatar_kwargs["agent_id"] = avatar_agent_id
        elif avatar_image_url:
            avatar_kwargs["agent_image_url"] = avatar_image_url
        else:
            raise ValueError("Set LEMONSLICE_AGENT_ID or LEMONSLICE_AVATAR_IMAGE_URL in env")

        avatar = lemonslice.AvatarSession(
            **avatar_kwargs,
            agent_prompt="a friendly animated 3D lion, warm and expressive, looking at the camera",
            agent_idle_prompt="a calm animated 3D lion looking at the camera with a gentle smile",
            idle_timeout=-1,
        )
        ...
        # Wait for LemonSlice to publish video track before greeting
        avatar_ready = asyncio.Event()

        @ctx.room.on("track_published")
        def on_track_published(publication: rtc.RemoteTrackPublication, participant: rtc.RemoteParticipant):
            if publication.kind == rtc.TrackKind.KIND_VIDEO and not participant.identity.startswith("user-"):
        ...
        session.generate_reply(
            instructions="Presentate como Leo, un simpático león animado y asistente virtual. Saluda brevemente y pregunta en que puedes ayudar."
        )
