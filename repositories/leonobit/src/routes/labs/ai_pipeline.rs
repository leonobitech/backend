...
impl AiPipeline {
  ...
  pub fn new(whisper_model_path: &str, elevenlabs_api_key: String) -> Result<Self, BoxError> {
    ...
    let whisper_ctx = Arc::new(
      WhisperContext::new_with_params(whisper_model_path, WhisperContextParameters::default())
    ...
    let elevenlabs_voice_id =
      std::env::var("ELEVENLABS_VOICE_ID").unwrap_or_else(|_| "EXAVITQu4vr4xnSDxMaL".to_string()); // "Rachel" común
  ...
  pub fn transcribe_audio(&self, pcm: &[f32], sample_rate_hz: u32, channels: u16) -> Result<String, BoxError> {
    ...
    for i in 0..num_segments {
      if let Some(seg) = state.get_segment(i as i32) {
        ...
        let seg_txt = match seg.to_str() {
          ...
          Err(_) => seg.to_str_lossy().map(|cow| cow.trim().to_owned()).unwrap_or_default(),
        };
