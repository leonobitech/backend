...
pub async fn handle_lab05(
  ...
  // -------------------------------------------------------------
  install_audio_loopback(&pc, local_track.clone(), local_ssrc, local_pt, cancel_pc.clone());
...
fn _spawn_ai_pipeline_example() -> Result<(), (StatusCode, String)> {
  let ai_pipeline = Arc::new(
    AiPipeline::new("models/ggml-base.en.bin", std::env::var("ELEVENLABS_API_KEY").unwrap()).map_err(internal)?,
  );
